...
/**
 * Send a notification message to the configured bot notification channel for a guild
 * @param client - Discord client instance
...
export async function sendBotNotification(
  ...
  try {
    ...
    logger.error(`Error sending bot notification to guild ${guildId}:`, error);
    return false;
  }
}

/**
 * Send a notification message to all configured bot notification channels
 * @param client - Discord client instance
 * @param message - The message content to send
...
export async function sendBotNotificationToAll(
  ...
  try {
    ...
    logger.error("Error sending bot notification to all guilds:", error);
    return 0;
  }
}
 ...
 */
export async function getGuildNotificationConfig(guildId: string) {
  try {
